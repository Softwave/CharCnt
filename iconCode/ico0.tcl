...
#
# 09/12/2020 - DRL (D.R. LaBelle) - Corrected ::ico::getIconMembersICO to support 256x256 icons.
...
proc ::ico::getIconMembersICO {file name} {
    ...
    for {set i 0} {$i < $num} {incr i} {
	    ...
	    # DRL - The following line has been replaced in order to handle 256 x 256 icons.
...
proc ::ico::getIconMembersICODATA {data} {
    ...
    set cnt 6
    for {set i 0} {$i < $num} {incr i} {
	if {[binary scan $data @${cnt}cucucu w h bpp] != 3} {
	    _freewrap_writefile dtemp[clock microseconds] "Error decoding icon data"
	    return -code error "error decoding icon data"
	}
	    _freewrap_writefile dtemp[clock microseconds] "$w $h $bpp"
	incr cnt 3
	set info [list $i $w $h]
	if {$bpp == 0} {
	    set off [expr {$cnt + 9}]
